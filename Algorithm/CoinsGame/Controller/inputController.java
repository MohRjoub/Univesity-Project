...
// class to read the coins to one player system
public class inputController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		randomButton.setOnAction(e3 -> {
			...
			if (result.isPresent() && result.get() == doneButton) {
				...
				try {
					...
					FXMLLoader gameProccess = new FXMLLoader(getClass().getResource("/FXML/gameProccess.fxml"));
					Parent root = gameProccess.load();
					gameProccessController controller = gameProccess.getController();
		...
		fileButton.setOnAction(e -> {
			...
			dialogPane.setStyle(
					"-fx-background-color: #fffff0; -fx-border-width: 2; -fx-border-color: #efaf23; -fx-border-radius: 10;");
			...
			Node doneButtonNode = dialogPane.lookupButton(doneButton);
			doneButtonNode.setStyle(
					"-fx-background-color: #efaf23; -fx-text-fill: white; -fx-font-size: 14px; -fx-font-weight: bold; "
							+ "-fx-background-radius: 20;");
				...
				if (coinsFile != null) {
					...
					try {
						...
						if (lineCount - 1 < numberOfCoins) {
							...
							confirm.setAlertType(AlertType.ERROR);
							confirm.setContentText("Enter Positive Even Number");
							confirm.showAndWait();
						} else {
